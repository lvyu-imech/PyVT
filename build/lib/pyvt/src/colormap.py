...
#_selected_cmaps = ["gray_r", "jet", "BuPu", "cool", "hot"]

_selected_cmaps = ["gray", "jet", "hsv", "rainbow", "cool", "hot", "brg", "gist_ncar", "twilight", \
                   "plasma", "viridis", "seismic", "YlOrBr", "YlGnBu", "Set3", "tab20", \
...
#	'''
	...
	#vmin = np.min(data)
	#vmax = np.max(data)
	#values = np.clip(data, vmin, vmax)
	#values -= vmin
	#values = values / (vmax - vmin)
	
    mp = cm_mpl.get_cmap(name=name)
    #return mp(value)[:, [0, 1, 2]]
    return mp(value)
