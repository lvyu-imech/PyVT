...
from .src.vector_form      import popup_widget_4VC
...
class AppForm(QMainWindow):
    ...
    def __init__(self):
        ...
        # find the path of icons
        import os
        import sys

        _path, _fname = os.path.split(__file__)
        self.iconpath = os.path.join(os.path.abspath(_path), 'icons')
    ...
    def write_plot(self):
        ...
        print('collect all available actor')
        if self.volumeActor.GetVisibility():
            print('actor I')
            actor_list.append(self.volumeActor)

        if (self.planeActor.GetProperty().GetOpacity()) != 0:
            print('actor II')
            actor_list.append(self.planeActor)

        if (self.isoActor.GetProperty().GetOpacity()) != 0:
            actor_list.append(self.isoActor)

        if (self.streamActor.GetProperty().GetOpacity()) != 0:
            actor_list.append(self.streamActor)

        if (self.glyphActor.GetProperty().GetOpacity()) != 0:
            actor_list.append(self.glyphActor)
         
        for item in self.module_list:
            if item.actor in self.renderer.GetActors():
                actor_list.append(item.actor)

        actor2d_list = [] 
        
        self.del_unused_actor()
        #text_list = []
        #for item in self.text_list:
        #    if item.actor in self.renderer.GetActors2D():
        ...
        render_with_actor(actor_list, actor2d_list, self.text_list, self.arrow_list, self.legend_list, \
                          self.frame_axes, self.orient_axes_widget, self.camera, self.outfilename, winsize, 2)
    ...
    def load_colormaps(self):
        ...
        for i in range(len(_selected_cmaps)):
            ...
            cmname = self.iconpath + '/cm_' + _selected_cmaps[i] + '.png' 
            icon = QIcon()
    ...
    def define_vector_variable(self):
        if self.output == None:
            ...
            self.popup_4VC = popup_widget_4VC(self.scalarnamelist)
    ...
    def control_cutplane_vis(self):
        if self.cutplane_see.isChecked():
            self.cutplane_see.setIcon(QIcon(self.iconpath + '/visible.png'))
            self.planeActor.SetVisibility(False) 
        else:
            self.cutplane_see.setIcon(QIcon(self.iconpath + '/invisible.png'))
    ...
    def control_isosurf_vis(self):
        if self.isosurf_see.isChecked():
            self.isosurf_see.setIcon(QIcon(self.iconpath + '/visible.png'))
            self.isoActor.SetVisibility(False)
        else:
            self.isosurf_see.setIcon(QIcon(self.iconpath + '/invisible.png'))
    ...
    def control_streamline_vis(self):
        if self.streamline_see.isChecked():
            self.streamline_see.setIcon(QIcon(self.iconpath + '/visible.png'))
            self.streamActor.SetVisibility(False) 
        else:
            self.streamline_see.setIcon(QIcon(self.iconpath + '/invisible.png'))
    ...
    def control_vectorfield_vis(self):
        if self.vectorfield_see.isChecked():
            self.vectorfield_see.setIcon(QIcon(self.iconpath + '/visible.png'))
            self.glyphActor.SetVisibility(False)
        else:
            self.vectorfield_see.setIcon(QIcon(self.iconpath + '/invisible.png'))
    ...
    def create_main_frame(self):
        ...
        # Other GUI controls
        #
        AddFile = QAction(QIcon(self.iconpath + '/open.png'), 'open file', self)  #
        AddFile.setShortcut('Ctrl+N')
        #AddFile.triggered.connect(self.browse_folder)
        AddFile.triggered.connect(self.open_file) 

        self.save_figure = QAction(QIcon(self.iconpath + '/save_figure.png'), 'save figure', self)
        self.save_figure.triggered.connect(self.save_plot)
        
        self.save_journal = QAction(QIcon(self.iconpath + '/save.png'), 'save journal', self)
        self.save_journal.triggered.connect(self.write_journal)
        
        self.orient_axes = QAction(QIcon(self.iconpath + '/orient_axes.png'), 'show orientation axes', self)
        self.orient_axes.triggered.connect(self.show_orient_axes)
       
        self.frame_axis = QAction(QIcon(self.iconpath + '/frame_axes.png'), 'show frame axes', self)
        self.frame_axis.triggered.connect(self.show_frame_axes)
        self.axes_signal.connect(self.adjust_frame_axes)

        self.view_x = QAction(QIcon(self.iconpath + '/x.png'), 'view YZ plane', self)
        self.view_x.setCheckable(True)
        self.view_x.triggered.connect(self.view_yz)

        self.view_y = QAction(QIcon(self.iconpath + '/y.png'), 'view YZ plane', self)
        self.view_y.setCheckable(True)
        self.view_y.triggered.connect(self.view_xz)

        self.view_z = QAction(QIcon(self.iconpath + '/z.png'), 'view XZ plane', self)
        self.view_z.setCheckable(True)
        self.view_z.triggered.connect(self.view_xy)
     
        self.outline = QAction(QIcon(self.iconpath + '/outline.png'), 'show outline', self)
        self.outline.triggered.connect(self.show_featureedge)
        
        self.boundbox = QAction(QIcon(self.iconpath + '/boundbox.png'), 'show bounding box', self)
        self.boundbox.triggered.connect(self.show_boundbox)

        self.mesh = QAction(QIcon(self.iconpath + '/mesh.png'), 'show mesh', self)
        self.mesh.triggered.connect(self.show_mesh)
        
        self.colorbar = QAction(QIcon(self.iconpath + '/colorbar.png'), 'show colorbar', self)
        self.colorbar.setCheckable(True)
        self.colorbar.triggered.connect(self.control_colorbar)

        self.background = QAction(QIcon(self.iconpath + '/background.png'), 'change background color', self)
        self.background.triggered.connect(self.change_bgcolor)

        self.reset = QAction(QIcon(self.iconpath + '/reset.png'), 'reset view', self)
        self.reset.triggered.connect(self.reset_camera)
     
        self.rot90 = QAction(QIcon(self.iconpath + '/rotate90deg.png'), 'rotate by 90deg', self)
        self.rot90.triggered.connect(self.rotate_ninty_deg)
     
        self.vector_define = QAction(QIcon(self.iconpath + '/define_vector.png'), 'define vector', self)
        self.vector_define.triggered.connect(self.define_vector_variable)

        self.scalar_define = QAction(QIcon(self.iconpath + '/define_scalar.png'), 'define scalar', self)
        ...
        self.text_add = QAction(QIcon(self.iconpath + '/text.png'), 'add text', self)
        self.text_add.setCheckable(True)
        self.text_add.triggered.connect(self.control_text)
        self.text_signal.connect(self.show_text)

        self.arrow_add = QAction(QIcon(self.iconpath + '/arrow.png'), 'add arrow', self)
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/cutplane.png'), QIcon.Normal, QIcon.Off)
        self.cutplane.setIcon(icon)
        self.cutplane.setIconSize(QSize(42, 24))
        self.cutplane_signal.connect(self.preview_cutplane) 
        self.cutplane_see       = QToolButton(self); self.cutplane_see.setIcon(QIcon(self.iconpath + '/invisible.png')) 
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/isosurf.png'), QIcon.Normal, QIcon.Off)
        self.isosurf.setIcon(icon)
        self.isosurf.setIconSize(QSize(42, 24))
        self.isosurf_signal.connect(self.preview_isosurf) 
        self.isosurf_see       = QToolButton(self); self.isosurf_see.setIcon(QIcon(self.iconpath + '/invisible.png')) 
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/streamline.png'), QIcon.Normal, QIcon.Off)
        ...
        self.streamline_see     = QToolButton(self); self.streamline_see.setIcon(QIcon(self.iconpath + '/invisible.png')) 
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/vectorfield.png'), QIcon.Normal, QIcon.Off)
        ...
        self.vectorfield_see    = QToolButton(self); self.vectorfield_see.setIcon(QIcon(self.iconpath + '/invisible.png')) 
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/preview.png'), QIcon.Normal, QIcon.Off)
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/clean.png'), QIcon.Normal, QIcon.Off)
        ...
        icon = QIcon() 
        icon.addPixmap(QPixmap(self.iconpath + '/add2basket.png'), QIcon.Normal, QIcon.Off)
    ...
    def change_bgcolor(self):
        ...
        if self.renderer.GetBackground() == (1.0, 1.0, 1.0):    # if background is white
            ...
            #change the font color of annotations
            for item in self.text_list:
                item.actor.GetScaledTextProperty().SetColor((1.0, 1.0, 1.0))
            for item in self.legend_list:
                item.actor.GetTitleTextProperty().SetColor((1.0, 1.0, 1.0))
                item.actor.GetLabelTextProperty().SetColor((1.0, 1.0, 1.0))
            for item in self.arrow_list:
                item.actor.GetProperty().SetColor(1.0, 1.0, 1.0)
            ...
            #change the font color of annotations
            for item in self.text_list:
                item.actor.GetScaledTextProperty().SetColor((0.0, 0.0, 0.0))
            for item in self.legend_list:
                item.actor.GetTitleTextProperty().SetColor((0.0, 0.0, 0.0))
                item.actor.GetLabelTextProperty().SetColor((0.0, 0.0, 0.0))
            for item in self.arrow_list:
                item.actor.GetProperty().SetColor(0.0, 0.0, 0.0)
    ...
    def rotate_ninty_deg(self):
        # rotate the screen 90 degree clockwise
        self.camera.Roll(-90)
        self.renderer.ResetCamera()
        self.vtkWidget.GetRenderWindow().Render()
    ...
    def create_menu(self):
        ...
        open_file_action = self.create_action("&Open", \
                           shortcut="Ctrl+N", slot=self.open_file)

        save_plot_action = self.create_action("&Save plot",  \
                           shortcut="Ctrl+S", slot=self.save_plot)

        write_journal_action = self.create_action("&Save journal", \
                           shortcut="Ctrl+J", slot=self.write_journal)
        ...
        self.add_actions(self.file_menu, \
                        (open_file_action, save_plot_action, \
                         write_journal_action, quit_action))
        
        ## View
        self.view_menu = self.menuBar().addMenu("&View")

        view_reset_action = self.create_action("&Reset", \
                            slot=self.reset_camera)

        view_x_action = self.create_action("&X-View", \
                         slot=self.view_yz)
        view_y_action = self.create_action("&Y-View", \
                         slot=self.view_xz)
        view_z_action = self.create_action("&Z-View", \
                         slot=self.view_xy)

        self.add_actions(self.view_menu, \
                         (view_reset_action, \
                          view_x_action, view_y_action, view_z_action)) 

        ## Define
        self.define_menu = self.menuBar().addMenu("&Define")
        
        define_scalar_action = self.create_action("&Scalar variable", \
                               slot=self.control_scalar_variable_vis) 
        
        define_vector_action = self.create_action("&Vector variable", \
                               slot=self.define_vector_variable)


        define_isosurf_action = self.create_action("&Iso-Surface", \
                                slot=self.define_isosurf)

        define_cutplane_action = self.create_action("&Cutplane", \
                                 slot=self.define_cutplane)

        define_streamline_action = self.create_action("&Streamline", \
                                   slot=self.define_streamline)

        define_vectorfield_action = self.create_action("&Vector field", \
                                    slot=self.define_vectorfield)

        self.add_actions(self.define_menu, \
                         (define_scalar_action, define_vector_action, \
                          define_isosurf_action, define_cutplane_action, \
                          define_streamline_action, define_vectorfield_action))
    ...
    def delete_module(self):
        ...
        for row in range(self.mcontainer.count()):
            print(row)
            if self.mcontainer.item(row).checkState() == Qt.Checked:
                # make sure module removed from the scene
                module = self.module_list[row]
                self.renderer.RemoveActor(module.actor)
                
                print('checked')
                # gather item to be removed
                itemsToRemove.append(self.mcontainer.item(row))
                
                # delete from module list
                self.module_list.remove(module)
                
        # remove items from gui container
        for i in reversed(range(self.mcontainer.count())):
            listItem = self.mcontainer.item(i)
            if listItem in itemsToRemove:
                itemRow = self.mcontainer.row(listItem)
                print(itemRow)
                self.mcontainer.takeItem(itemRow)
