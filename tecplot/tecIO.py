...
#        raise RuntimeError('this data member has been removed')
    ...
    def _read_zonetype(self, zone, zone_type, lines, iline, iblock, headers_dict, line,
        ...
        if iblock == 0:
            ...
            print('zone.variables = %s' % zone.variables)
            nresults = len(variables) - 3 # x, y, z, rho, u, v, w, p
            #self.log.debug('nresults = %s' % nresults)
            print('nresults = %s' % nresults)

        #self.log.debug(str(headers_dict))
        print(str(headers_dict))
        is_unstructured = False
        is_structured = False
        if zone_type in ['FETRIANGLE', 'FEQUADRILATERAL', 'FETETRAHEDRON', 'FEBRICK']:
            nnodesi = headers_dict['Nodes']
            nelementsi = headers_dict['Elements']
        ...
        print(f'zone_type={zone_type} data_packing={data_packing} '
                      f'nnodes={nnodesi} nelements={nelementsi}')
        ...
        print('nnodes=%s nelements=%s (0-based)' % (nnodes, nelements))
        del headers_dict
        iblock += 1
        if iblock == 10:
            return
        #self.log.debug('final sline=%s' % sline)
        print('final sline=%s' % sline)
        return iline
    ...
    for iheader, header in enumerate(headers):
        ...
        if parse:
            ...
            allowed_keys = ['VARIABLES', 'T', 'ZONETYPE', 'DATAPACKING', # 'TITLE',
                            'NODES', 'ELEMENTS', 'F', 'DT', 'SOLUTIONTIME', 'STRANDID',
                            'I', 'J', 'K']
...
def _simplify_header(headers_dict, variables: List[str]) -> None:
    ...
    # unstructured
    if 'NODES' in headers_dict: # nnodes
        headers_dict['NODES'] = int(headers_dict['NODES'])
    if 'ELEMENTS' in headers_dict: # nelements
        headers_dict['ELEMENTS'] = int(headers_dict['ELEMENTS'])
