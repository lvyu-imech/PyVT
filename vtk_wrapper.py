...
def render_with_actor(actor_list, actor2D_list, text_list,  \
    ...
    for legend in legend_list:
        ...
        legend.actor.GetLabelTextProperty().SetFontSize(fontsize * scale) 
    ...
    for legend in legend_list:
        fontsize = legend.actor.GetTitleTextProperty().GetFontSize()
        legend.actor.GetTitleTextProperty().SetFontSize(int(fontsize / scale))
        fontsize = legend.actor.GetLabelTextProperty().GetFontSize()
        legend.actor.GetLabelTextProperty().SetFontSize(int(fontsize / scale))
...
def render_script_mode(journal_file):
    ...
    writer = vtk.vtkPostScriptWriter()
    #writer = vtk.vtkPNGWriter()
    writer.SetFileName('test_fig.eps')
    writer.SetInputConnection(w2if.GetOutputPort())
    writer.Write()
